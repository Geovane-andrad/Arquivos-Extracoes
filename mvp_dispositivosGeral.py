...
def get_dispositivos_geral_mvp():
    ...
    for pagina in range(1, last_page + 1):
        ...
        Dispositivos_geral = dados.get("data", [])  # Agora estabelecimentos é uma lista
        if not Dispositivos_geral:
            print(f"Sem dados na página {pagina}. Continuando para a próxima...")
            continue

        print(f"Página {pagina} processada com {len(Dispositivos_geral)} estabelecimentos. ", pagina, "/", last_page)
        print("*" * 50)

        todas_paginas.extend(Dispositivos_geral)

    # Cria o DataFrame com todas as páginas
    df_Dispositivos_geral = pd.DataFrame(todas_paginas)

    # Trata caracteres especiais e valores nulos
    df_Dispositivos_geral = df_Dispositivos_geral.applymap(
    ...
    df_Dispositivos_geral_col_validas = df_Dispositivos_geral[colunas_validas]

    # Trata campos de texto para evitar problemas com aspas
    df_Dispositivos_geral_col_validas["razao_social"] = df_Dispositivos_geral_col_validas["razao_social"].astype(str).apply(
        lambda x: x.replace("'", "").replace('"', "") if isinstance(x, str) else x
    )

    try:
        # Inserindo os registros no banco de dados
        fn_inserirRegistros(cnx.conn, df_Dispositivos_geral_col_validas, "extract.mvp_dispositivosgeral")
        run_procedure('extract.sp_merger_mvp_dispositivosgeral()')
