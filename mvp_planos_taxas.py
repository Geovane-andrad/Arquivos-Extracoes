...
def get_mvp_planos_taxas():
    projeto = "'carga datalake'"
    etapa = "'extração'"
    script = "'get_mvp_planos_taxas.py'"
    pid = "'airflow'"
    ...
    try:
        ...
        df_taxas["Nome"] = df_taxas["Nome"].astype(str).apply(
        lambda x: x.replace("'", "").replace('"', "") if isinstance(x, str) else x
        )
        df_taxas["Descricao"] = df_taxas["Descricao"].astype(str).apply(
        lambda x: x.replace("'", "").replace('"', "") if isinstance(x, str) else x
        ) 
        ...
        fn_inserirRegistros(cnx.conn, df_final, "extract.mvp_plano_taxas")
       # run_procedure('extract.sp_merger_mvp_taxas_planos()')
